...
from pages.app import APP_ENV, APP_VERSION, _strategy_catalog_is_ready, app, display_page, server
...
class ServiceEndpointTests(unittest.TestCase):
    def test_catalog_readiness_rejects_duplicates_and_metadata_gaps(self):
        names = [f"Strategy{index}" for index in range(100)]
        profiles = {name: {} for name in names}
        self.assertTrue(_strategy_catalog_is_ready(names, profiles))
        self.assertFalse(_strategy_catalog_is_ready(names + [names[0]], profiles))
        self.assertFalse(_strategy_catalog_is_ready(names, {**profiles, "Unregistered": {}}))
        self.assertTrue(
            _strategy_catalog_is_ready(
                names + ["Strategy100"],
                {**profiles, "Strategy100": {}},
            )
        )
    ...
    def test_liveness_and_readiness_endpoints(self):
        ...
        self.assertEqual(ready.get_json()["strategy_count"], 100)
