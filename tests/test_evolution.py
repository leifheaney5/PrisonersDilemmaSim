...
class EvolutionTests(unittest.TestCase):
    ...
    def test_evolution_result_rejects_non_finite_and_out_of_range_metrics(self):
        config = EvolutionConfig(
            strategy_names=("A", "B"),
            initial_shares={"A": 0.5, "B": 0.5},
            generations=1,
        ...
        original = build_evolution_result(config, evolve_population(["A", "B"], stats, generations=1), stats)
        cases = (
            (("fitness", "A"), float("nan"), "fitness"),
            (("average_payoff",), 6.0, "average_payoff"),
            (("cooperation_rate",), -0.1, "cooperation_rate"),
            (("diversity",), float("inf"), "diversity"),
        )
        for path, value, message in cases:
            with self.subTest(path=path):
                artifact = json.loads(json.dumps(original))
                target = artifact["history"][0]
                if len(path) == 2:
    ...
    def test_evolution_result_requires_complete_bounded_matchup_statistics(self):
        config = EvolutionConfig(
            strategy_names=("A", "B"),
            initial_shares={"A": 0.5, "B": 0.5},
            generations=1,
        ...
        missing["matchup_statistics"].pop()
        with self.assertRaisesRegex(ValueError, "missing"):
            validate_evolution_result(missing)

        duplicate = json.loads(json.dumps(original))
        duplicate["matchup_statistics"][1] = dict(duplicate["matchup_statistics"][0])
        with self.assertRaisesRegex(ValueError, "duplicate"):
            validate_evolution_result(duplicate)

        invalid = json.loads(json.dumps(original))
        invalid["matchup_statistics"][0]["cooperation"] = 2.0
        with self.assertRaisesRegex(ValueError, "cooperation"):
            validate_evolution_result(invalid)

    def test_moran_result_requires_consistent_integer_population_counts(self):
        config = EvolutionConfig(
            strategy_names=("A", "B"),
            initial_shares={"A": 0.5, "B": 0.5},
            generations=1,
        ...
        history = evolve_moran_population(["A", "B"], stats, generations=1, population_size=20)
        original = build_evolution_result(config, history, stats)

        invalid_total = json.loads(json.dumps(original))
        invalid_total["history"][0]["counts"]["A"] += 1
        with self.assertRaisesRegex(ValueError, "sum to population_size"):
            validate_evolution_result(invalid_total)

        invalid_type = json.loads(json.dumps(original))
        invalid_type["history"][0]["counts"]["A"] = 10.5
        with self.assertRaisesRegex(ValueError, "integers"):
            validate_evolution_result(invalid_type)
