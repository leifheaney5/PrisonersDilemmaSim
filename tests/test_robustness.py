...
class SeedSweepTests(unittest.TestCase):
    ...
    def test_seed_sweep_result_rejects_missing_and_invalid_rows(self):
        ...
        invalid_rounds = json.loads(json.dumps(result))
        invalid_rounds["seed_results"][0]["rounds"] += 1
        with self.assertRaisesRegex(ValueError, "configured participation count"):
            validate_seed_sweep_result(invalid_rounds)

    def test_seed_sweep_self_play_participation_counts_both_copies(self):
        result = run_seed_sweep(
            SeedSweepConfig(
                strategy_names=("TitForTat", "BadCop"),
                seed_count=1,
        ...
        restored = validate_seed_sweep_result(result)
        self.assertEqual({row["rounds"] for row in restored["seed_results"]}, {18})
