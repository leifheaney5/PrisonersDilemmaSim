...
STRATEGY_PROFILES: dict[str, dict[str, str]] = {
    ...
    "Shootout": {
        "description": "Opens with defection, then alternates between cooperation and defection.",
        "origin": "Project-defined alternating duel strategy.",
        "notes": "Creates a predictable D/C rhythm that is distinct from BadAlternator.",
    },
    "ParkBus": {
...
_EXTENDED_PROFILE_SPECS = {
    "ReverseTitForTat": ("Cooperates first, then plays the opposite of the opponent's previous move.", "Challenge predictable reciprocity by deliberately taking the contrary action."),
    "TitForThreeTats": ("Defects only after three consecutive opponent defections.", "Give repeated benefit of the doubt before retaliating."),
    "ThreeTitsForTat": ("Defects for up to three responses after any recent opponent defection.", "Make a single betrayal carry a longer but finite consequence."),
    "DelayedTitForTat": ("Cooperates twice, then copies the opponent's move from two rounds earlier.", "Respond cautiously instead of reacting immediately."),
    "SlowTitForTat": ("Retaliates after defection and requires two cooperative moves before restoring trust.", "Trust is lost quickly and rebuilt through consistent repair."),
    "OmegaTitForTat": ("Mirrors the opponent but cooperates to break detected alternating deadlocks.", "Reciprocity should yield when it traps both players in a pointless cycle."),
    "ThreeStrikes": ("Issues one retaliatory defection after every third cumulative opponent defection.", "Repeated offenses trigger milestone consequences without creating a permanent enemy."),
    "LimitedGrudge": ("Punishes a newly observed defection for five rounds, then forgives.", "Consequences should be serious but never permanent."),
    "ForgivingGrim": ("Uses a grim trigger but releases the grudge after three consecutive cooperations.", "Even severe distrust can be repaired through sustained good behavior."),
    "SoftGrudger": ("Answers defection with four defections followed by two cooperative repair moves.", "Punish firmly, then make reconciliation explicit."),
    "RollingTrust5": ("Cooperates when at least 60% of the last five observed moves were cooperative.", "Judge relationships by a short and responsive recent record."),
    "RollingTrust10": ("Cooperates when at least 60% of the last ten observed moves were cooperative.", "Use a broader recent record so trust changes more gradually."),
    "DecayingTrust": ("Maintains an exponentially weighted trust score and cooperates above 55%.", "Recent conduct matters most without erasing the past instantly."),
    "RecentMajority3": ("Copies the cooperative or defective majority in the last three moves, favoring cooperation on ties.", "Follow the opponent's immediate local tendency."),
    "RecentMajority7": ("Copies the cooperative or defective majority in the last seven moves, favoring cooperation on ties.", "Demand a longer pattern before changing disposition."),
    "ProportionalRetaliator": ("Issues roughly one retaliatory defection for every two opponent defections.", "Punishment should be measurable and proportional."),
    "RestorativeJustice": ("Retaliates once after defection and then immediately offers cooperation.", "Answer harm while always opening a path to repair."),
    "AdaptiveAspiration": ("Keeps successful actions relative to a moving payoff aspiration and switches after disappointment.", "Adapt expectations as the relationship reveals what outcomes are attainable."),
    "SuspiciousPavlov": ("Uses Win-Stay, Lose-Shift but opens with defection.", "Begin guarded, then learn from outcomes."),
    "GenerousPavlov": ("Uses Pavlov but forgives 20% of planned defections.", "Outcome learning works better when occasional mercy can reset conflict."),
    "WinShiftLoseStay": ("Switches actions after high payoffs and stays after low payoffs.", "Explore after success and persist through adversity."),
    "Grofman": ("Cooperates after matching actions and otherwise cooperates with probability 2/7.", "Agreement invites trust while disagreement calls for cautious random repair."),
    "RandomCooperator75": ("Cooperates independently with 75% probability each round.", "Lean toward goodwill without becoming predictable."),
    "RandomDefector75": ("Defects independently with 75% probability each round.", "Prefer exploitation while retaining occasional cooperation."),
    "PeriodicCCD": ("Repeats the sequence Cooperate, Cooperate, Defect.", "Build two rounds of value before taking one opportunistic turn."),
    "PeriodicDDC": ("Repeats the sequence Defect, Defect, Cooperate.", "Offer cooperation sparingly within an otherwise guarded schedule."),
    "PeriodicCCCD": ("Repeats three cooperations followed by one defection.", "Maintain a mostly cooperative rhythm with periodic exploitation."),
    "PeriodicCDDD": ("Repeats one cooperation followed by three defections.", "Use a small cooperative signal inside a strongly defensive rhythm."),
    "Detective": ("Tests with D-C-C-D, then mirrors retaliation or exploits an opponent that never pushed back.", "Probe boundaries before deciding whether reciprocity or exploitation pays."),
    "HardProber": ("Tests with D-D-C-C, then mirrors retaliators and defects against passive opponents.", "Apply a harsher test before granting reciprocal treatment."),
    "NaiveProber": ("Opens C-C-D and then follows Tit-for-Tat regardless of the response.", "Explore once but remain committed to transparent reciprocity."),
    "ExploitThenCopy": ("Defects twice and then follows Tit-for-Tat.", "Take an early advantage before settling into reciprocity."),
    "CooperateThenGrim": ("Cooperates for five rounds, then permanently punishes any observed defection.", "Establish trust first, but treat betrayal as decisive."),
    "ScheduledForgiveness": ("Follows Tit-for-Tat but cooperates unconditionally every fifth round.", "Create regular opportunities for a damaged relationship to reset."),
    "ScheduledDefection7": ("Follows Tit-for-Tat but defects unconditionally every seventh round.", "Test resilience periodically even inside reciprocal play."),
    "PrimeCooperator": ("Cooperates on prime-numbered rounds and defects otherwise.", "Let a mathematical schedule create sparse, irregular trust signals."),
    "SquareDefector": ("Defects on perfect-square rounds and cooperates otherwise.", "Keep cooperation dominant while reserving predictable challenges."),
    "FibonacciCooperator": ("Cooperates on Fibonacci-numbered rounds and defects otherwise.", "Use an expanding mathematical rhythm to make cooperation increasingly sparse."),
    "FairnessSeeker": ("Cooperates while tied or ahead in cumulative payoff and defects while behind.", "Protect relative fairness rather than maximizing score in isolation."),
    "AlternatingTitForTat": ("Alternates between mirroring and opposing the opponent's previous move.", "Balance reciprocity with deliberate unpredictability."),
}

for _name, (_description, _philosophy) in _EXTENDED_PROFILE_SPECS.items():
    STRATEGY_PROFILES[_name] = {
        "description": _description,
        "origin": "Project-defined extension strategy.",
        "notes": _philosophy,
        "philosophy": _philosophy,
    }

EXTENDED_STRATEGY_NAMES = frozenset(_EXTENDED_PROFILE_SPECS)
EXTENDED_STOCHASTIC = frozenset({"GenerousPavlov", "Grofman", "RandomCooperator75", "RandomDefector75"})
EXTENDED_TIME_BASED = frozenset({
    "PeriodicCCD", "PeriodicDDC", "PeriodicCCCD", "PeriodicCDDD", "Detective", "HardProber",
    "NaiveProber", "ExploitThenCopy", "CooperateThenGrim", "ScheduledForgiveness", "ScheduledDefection7",
    "PrimeCooperator", "SquareDefector", "FibonacciCooperator", "AlternatingTitForTat",
})
EXTENDED_ZERO_MEMORY = frozenset({
    "RandomCooperator75", "RandomDefector75", "PeriodicCCD", "PeriodicDDC", "PeriodicCCCD", "PeriodicCDDD",
    "PrimeCooperator", "SquareDefector", "FibonacciCooperator",
})
...
def strategy_scorecard(name: str) -> dict[str, object]:
    ...
    if nm in EXTENDED_STRATEGY_NAMES:
        return {
            "deterministic": nm not in EXTENDED_STOCHASTIC,
            "stochastic": nm in EXTENDED_STOCHASTIC,
            "memory": 0 if nm in EXTENDED_ZERO_MEMORY else "stateful",
            "primarily_cooperative": nm not in {"RandomDefector75", "PeriodicDDC", "PeriodicCDDD", "HardProber"},
            "primarily_defective": nm in {"RandomDefector75", "PeriodicDDC", "PeriodicCDDD", "HardProber"},
            "reactive": nm not in EXTENDED_ZERO_MEMORY,
            "time_based": nm in EXTENDED_TIME_BASED,
            "horizon_aware": False,
        }
