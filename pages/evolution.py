...
def normalize_evolution_config(config: EvolutionConfig) -> EvolutionConfig:
    ...
    pairings = len(names) * (len(names) + 1) // 2
    workload = pairings * rounds * repetitions
...
def validate_evolution_result(payload: object, known_strategies: Sequence[str] | None = None) -> dict[str, object]:
    ...
    generations = [int(row.get("generation", -1)) for row in history if isinstance(row, Mapping)]
    ...
    for row in history:
        ...
        fitness = row.get("fitness") if isinstance(row, Mapping) else None
