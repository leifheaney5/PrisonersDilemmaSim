...
def normalize_seed_sweep_config(config: SeedSweepConfig) -> SeedSweepConfig:
    ...
    workload = seed_count * tournament_workload(
        len(names),
        rounds,
        repetitions,
...
def validate_seed_sweep_result(payload: object, known_strategies: Sequence[str] | None = None) -> dict[str, object]:
    ...
    opponent_appearances = len(config.strategy_names) - 1 + (2 if config.include_self_play else 0)
    expected_rounds = config.rounds_per_match * config.repetitions * opponent_appearances
    ...
    for raw_row in raw_rows:
        ...
        if rounds != expected_rounds:
            raise ValueError(f"seed result rounds must equal the configured participation count ({expected_rounds})")
