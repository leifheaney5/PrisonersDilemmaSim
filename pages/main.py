...
class Game:
    ...
    def play_game(self, strategy1, strategy2):
        ...
        # Store each round's result and moves
...
app.layout = html.Div(children=[
    html.H1(children='Game Theory Simulation Results'),
    html.H2(children='Prisoners Dilemma'),
    dcc.Graph(
        id='total-points',
        figure=go.Figure(
            data=[
    ...
    dcc.Graph(
        ...
        figure=go.Figure(
            ...
            layout=go.Layout(title='Biggest Win per Strategy (turn-based)')
        )
    ),
    dcc.Graph(
        id='biggest-loss',
        figure=go.Figure(
            ...
            layout=go.Layout(title='Biggest Loss per Strategy (turn-based)')
        )
    ),
    dcc.Dropdown(
...
def set_opponent_options(selected_strategy):
    # Find all strategies that have played against the selected strategy
