...
The app includes 100 built-in strategies. They range from standard IPD baselines
...
- **Extended reciprocity:** ReverseTitForTat, TitForThreeTats, ThreeTitsForTat,
  DelayedTitForTat, SlowTitForTat, OmegaTitForTat, and AlternatingTitForTat
- **Trust, punishment, and restoration:** ThreeStrikes, LimitedGrudge, ForgivingGrim,
  SoftGrudger, DecayingTrust, ProportionalRetaliator, and RestorativeJustice
- **Recent evidence:** RollingTrust5, RollingTrust10, RecentMajority3, and RecentMajority7
- **Outcome learning:** AdaptiveAspiration, SuspiciousPavlov, GenerousPavlov,
  WinShiftLoseStay, Grofman, and FairnessSeeker
- **Random and periodic controls:** RandomCooperator75, RandomDefector75, PeriodicCCD,
  PeriodicDDC, PeriodicCCCD, and PeriodicCDDD
- **Testing and phase policies:** Detective, HardProber, NaiveProber, ExploitThenCopy,
  CooperateThenGrim, ScheduledForgiveness, and ScheduledDefection7
- **Mathematical schedules:** PrimeCooperator, SquareDefector, and FibonacciCooperator
...
Each of the 40 extended profiles also separates its exact game rule from a concise
philosophy statement explaining the strategic attitude behind that behavior.
...
an initial public-preview safeguard. Set `REDIS_URL` to use the atomic shared
sliding-window limiter across every web worker. A background worker remains required
before increasing public experiment workloads.
...
interaction, the 100-strategy landscape, primary experiment modes, service health,
...
After deploying staging or production, verify its public routes, health contract,
100-strategy catalog, version metadata, and security headers with:

```bash
python -m scripts.verify_deployment https://your-deployment.example
```

Maintainers can run the same release gate from GitHub Actions using the
**Verify deployment** workflow. Public deployment checks require HTTPS; loopback
HTTP remains available for local validation.
...
## Repo notes
- Contribution and pull request guidance, including how to consolidate overlapping
  branches, is documented in [`CONTRIBUTING.md`](CONTRIBUTING.md).
...
- Core simulation logic lives in `pages/game_logic.py`; reusable tournament analytics
  live in `pages/analytics.py`; shared round-robin workload estimates live in
  `pages/workloads.py`; and Dash-independent robustness and evolution result
  presentation lives in `pages/experiment_views.py`. Batch, live, robustness, and
  evolution configurations use the same pairing and round-count semantics. The Dash UI and callback
  registration remain in `pages/app.py` while the larger layout/callback refactor
  continues incrementally.
- Uploaded JSON artifacts are decoded through `pages/artifact_io.py`, which enforces
  byte, nesting, collection-size, duplicate-key, UTF-8, and finite-number rules before
  experiment-specific schema validation runs.
- User-visible failures carry a request reference while `pages/error_reporting.py`
  logs the corresponding operation code, exception type, and traceback. Validation
  details remain actionable but bounded; unexpected internal messages are not exposed.
