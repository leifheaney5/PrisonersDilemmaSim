...
5. Use the repository merge queue when it is enabled so the final combined
   commit is tested against the current default branch before landing.
...
CI runs for pull requests and merge-queue groups. It also scans source,
documentation, workflow, and browser-test files for unresolved conflict markers.
